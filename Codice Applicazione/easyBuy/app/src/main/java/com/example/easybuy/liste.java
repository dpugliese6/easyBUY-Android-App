...
public class liste extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        titolo.setText("Liste");
        crea = (Button) findViewById(R.id.bottone_aggiungi);
        lista_activity = (ListView) findViewById(R.id.lista_carrelli_liste);

        List lista = db.getListeFromUtente(db.getEmail());

        if (lista.size() != 0) {
            liste_adapter adapter = new liste_adapter(this, R.layout.riga, lista);
            lista_activity.setAdapter(adapter);
        } else {
            List objects = new LinkedList();
            objects.add("un elemento giusto per fare uscire una riga");
            non_ci_sono_elementi_adapter a = new non_ci_sono_elementi_adapter(this, R.layout.non_ci_sono_liste, objects);
        ...
        crea.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                Intent intent = new Intent(liste.this, crea_lista_carrello.class);
    ...
    public void onBackPressed() {
        Intent intent = new Intent(liste.this, home.class);
        startActivity(intent);
        finish();
    }
}
